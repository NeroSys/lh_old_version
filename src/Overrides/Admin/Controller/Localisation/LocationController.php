...
namespace App\Overrides\Admin\Controller\Localisation;

require_once(DIR_OPENCART . 'admin/controller/localisation/location.php');

use App\Overrides\Catalog\Model\Localisation;

class LocationController extends \ControllerLocalisationLocation
{
    ...
    protected function getList() {
        ...
        $model = new Localisation\LocationModel($this->registry);
        $results = $model->getLocations($filter_data);

        foreach ($results as $result) {
            $location = $result;
            $location["edit"] = $this->url->link('localisation/location/edit', 'token=' . $this->session->data['token'] . '&location_id=' . $result['location_id'] . $url, true);
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['id_erp'])) {
            $data['id_erp'] = $this->request->post['id_erp'];
        } elseif (!empty($location_info)) {
            $data['id_erp'] = $location_info['id_erp'];
        } else {
            $data['id_erp'] =   'Уникальный ид с 1с'.mt_rand(1,99999);
        }
